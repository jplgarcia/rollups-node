...
func NewPostgresEventsService(pool *pgxpool.Pool) events.EventsService {
	return &pgEventsService{pool: pool}
}

func (p *pgEventsService) Publish(ctx context.Context, e events.Event) error {
	...
	_, err = p.pool.Exec(ctx, "SELECT pg_notify($1::text, $2::text)", notifyChannel, string(b))
	return err
}
