...
func getIDHash(one, two *[32]byte) common.Hash {
	bytes32Type, _ := abi.NewType("bytes32", "", nil)
	args := abi.Arguments{
		{Type: bytes32Type},
		{Type: bytes32Type},
	}
	packed, err := args.Pack(*one, *two)
	if err != nil {
		panic(fmt.Sprintf("failed to pack ABI for getIdHash: %v", err))
	}
...
func (s *Service) saveCommitments(ctx context.Context, app *Application, epoch *Epoch,
	tournament *Tournament, events []CommitmentJoinedEvent) error {
	for _, ev := range events {
		...
		s.Logger.Info("CommitmentJoined event",
			...
			"commitment", fmt.Sprintf("%x", c.Commitment))
...
func (s *Service) saveMatches(ctx context.Context, app *Application, epoch *Epoch,
	tournament *Tournament, events []MatchCreatedEvent) error {
	for _, ev := range events {
		one := ev.One()
		two := ev.Two()
		...
		m := Match{
			...
			EpochIndex:        epoch.Index,
			TournamentAddress: tournament.Address,
			IDHash:            getIDHash(&one, &two),
			CommitmentOne:     one[:],
			CommitmentTwo:     two[:],
			LeftOfTwo:         leftOfTwo[:],
			WinnerCommitment:  nil,
			BlockNumber:       ev.BlockNumber(),
			TxHash:            ev.TxHash(),
		}
		s.Logger.Info("MatchCreated event",
			...
			"one", fmt.Sprintf("%x", m.CommitmentOne),
			"two", fmt.Sprintf("%x", m.CommitmentTwo),
			"leftOfTwo", fmt.Sprintf("%x", m.LeftOfTwo))
...
func (s *Service) saveMatchAdvanced(ctx context.Context, app *Application, epoch *Epoch,
	tournament *Tournament, events []MatchAdvancedEvent) error {
	for _, ev := range events {
		parent := ev.Parent()
		left := ev.Left()
		m := &MatchAdvanced{
		...
		s.Logger.Info("MatchAdvanced event",
			...
			"tournament", tournament.Address.Hex(),
			"id_hash", m.IDHash.String(),
			"parent", fmt.Sprintf("%x", m.Parent),
			"left_node", fmt.Sprintf("%x", m.LeftNode))
...
func (s *Service) saveMatchDeleted(_ context.Context, app *Application, epoch *Epoch,
	tournament *Tournament, events []MatchDeletedEvent) error {
	for _, ev := range events {
		// For now, just log
		s.Logger.Info("MatchAdvanced event",
			"application", app.Name,
			"epoch_index", epoch.Index,
			"tournament", tournament.Address.Hex(),
			"id_hash", ((common.Hash)(ev.Arg0())).String(),
		)
	}
	return nil
}
...
func (s *Service) fetchRootTournamentData(ctx context.Context, app *Application, epoch *Epoch) error {
	...
	adapter, err := NewTopTournamentAdapter(*epoch.TournamentAddress, ethClient, s.filter)
	if err != nil {
	...
	_, winnerCommitment, err := adapter.Result(nil)
	if err != nil {
	...
	for _, newInner := range events.NewInnerTournament {
		hashID := (common.Hash)(newInner.Arg0())
		middleAddress := newInner.Arg1()
...
func (s *Service) fetchMiddleTournamentData(ctx context.Context, app *Application, epoch *Epoch, hashID *common.Hash, middleAddress *common.Address) error {
	...
	adapter, err := NewMiddleTournamentAdapter(*middleAddress, ethClient, s.filter)
	if err != nil {
...
func (s *Service) fetchBottomTournamentData(
	...
	adapter, err := NewBottomTournamentAdapter(*bottomAddress, ethClient, s.filter)
	if err != nil {
