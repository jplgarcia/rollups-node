...
func (r *PostgresRepository) CreateMatch(
	...
	if len(m.WinnerCommitment) != 0 {
		winnerCommitment = postgres.Bytea(m.WinnerCommitment)
	}
	...
	selectQuery := table.Application.SELECT(
		...
		postgres.Bytea(m.CommitmentOne),
		postgres.Bytea(m.CommitmentTwo),
		postgres.Bytea(m.LeftOfTwo),
		winnerCommitment,
	...
	insertStmt := table.Matches.INSERT(
		...
		table.Matches.WinnerCommitment,
